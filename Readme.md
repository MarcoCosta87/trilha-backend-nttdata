...
## Responsabilidades das Camadas a seguir:

- **entity**: Na entity tem como responsabilidade, de determina os estados e possiveis comportamentos que os objetos podem ter,representando um conceito do seu modelo as entidades interligam-se atraves de mapeamento com banco de dados , definindo como as classes de entidades e seus atributos serão mapeados para tabelas e colunas do banco de dados.

- **controller**: São objetos de classe responsaveis pelo processamento das requisições e por gerar respostas, analisando os dados de entrada obtidos pela requisiçao. ela serve de intermediario de organizar os eventos da interface com usuario para acamada de modelo.

- **repository** : A camada repository tem como responsabilade de representar a camada de dados na aplicação. este repositorio faria a interface de criar, procurare guarda podendo incluir 
metodos especificos como filtros.

- **service** : classes que contem anotações @Service, são responsaveis por escrever a logica de negocios existetes em criar, atualizar e excluir.Uma camada separada do arquivo da classe controller. 